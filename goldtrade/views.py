...
def add_money(request):
    ...
    def staff_withdrawals(request):
        q = request.GET.get("q", "").strip()
        qs = Transaction.objects.filter(transaction_type="WITHDRAW").select_related("wallet__user").order_by(
            "-timestamp")
        if q:
    ...
    def approve_withdrawal(request, pk):
        ...
        def approve_deposit(request, pk):
            ...
            messages.success(request,
                             f"✅ Deposit approved. Rs. {deposit.amount:,.2f} credited to {deposit.user.username}.")
            return redirect("staff_deposits")
